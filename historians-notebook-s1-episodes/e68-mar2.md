---
title: Episode 68
author: Jonathan Seyfried
location: Albuquerque, NM
thumbnail: /historians-notebook-s1-episodes/images/mar-2-aca-cr-r1751-f16v-joan.jpg
summary: March 2, 1387 
date: 2026-03-02
layout: scrollstory
header-image: /historians-notebook-s1-episodes/images/molt-cara-companyona2.png
audio: /historians-notebook-s1-episodes/audio/s1e68-mar2.mp3
tags:
 - Joan
 - Marquis de Villena
 - Enrique de Villena
 - digitization

---

# Season 1, Episode 68
<a href="/historians-notebook/" target="Return to episodes list" style="float:right; margin-left:20px; margin-bottom:20px; text-align:center; max-width:300px; border:2px solid #212529; border-radius:12px; padding:15px; background-color:#355145ff; box-shadow:0 2px 8px  #E6E3C8; text-decoration:none; display:block; transition:transform 0.2s, box-shadow 0.2s;">
  <img src="/historians-notebook-s1-episodes/images/molt-cara-companyona-image.jpg" alt="Return to episodes list" style="width:200px;height:auto; display:block; margin:0 auto;">
  <span style="font-size:0.9em; color: #E6E3C8; margin-top:10px; display:block;">Return to episodes list</span>
</a>

Joan summons the Marquis de Villena to Barcelona and awards Alfonso the Younger with the Cross of Saint George.

<div class="audio-player-card">
  {% include audio.html src="/historians-notebook-s1-episodes/audio/s1e68-mar2.mp3" title="Episode 68" %}
</div>
<style>
.audio-player-card {
  border: 1px solid #212529;
  border-radius: 12px;
  padding: 20px;
  margin: 20px 0;
  width: 50%;
  background-color: #355145ff;
  box-shadow: 0 2px 8px rgba(0,0,0,0.1);
}
.audio-player-card .audio-title {
  margin: 0 0 10px 0;
  font-weight: bold;
  font-size: 1.1em;
}
.audio-player-card audio {
  width: 100%;
  display: block;
  margin: 0 auto;
}
</style>


<br style="clear: both">

{% include figure.html
  class="right"
  width="40%"
  caption="ACA CR R1751 f16v [Source: PARES](https://pares.mcu.es/ParesBusquedas20/catalogo/show/4801766)"
  image-path="/historians-notebook-s1-episodes/images/mar-2-aca-cr-r1751-f16v-joan.jpg"
%}


## Today’s Document 
- Subject: Joan summons for the Marquis de Villena
- Date: March 2, 1387
- Day of the Week: Saturday
- Language: Catalan
- Archival Reference Number: ACA CR R1751 ff16v
- [Link to PARES](https://pares.mcu.es/ParesBusquedas20/catalogo/show/4801766)
- Place: Barcelona
- Sender: Joan
- Recipient: Alfonso the Elder, the Marquis of Villena

## Historical Thinking Notes
- Sourcing: Alfonso the Elder was a grandson of James II of Aragon, Joan's great-grandfather; Alfonso the Elder had supported Pere the Ceremonious in many wars; writing to a member of the royal family, Joan perhaps summons Alfonso to Barcelona in an effort to coordinate the exercise of power stemming from his own authority as king

- Contextualization: kings and queens relied on the loyalty of other members of the royal family as power brokers and administrators in the Late Middle Ages; chivalric orders like the 'empresa' of the Cross of Saint George mentioned in this letter helped to motivate and organize members of the nobility in service of the king

- Corroboration: the other letters of summons examined in [Episode 16](e16-jan9), [Episode 19](e19-jan12), and [Episode 60](e60-feb22) offer valuable comparisons to the one from this episode; the language deployed in the summons genre varies in urgency 

- Close-Reading: in line 3, Joan wrote that he excused Alfonso from the expectation of an earlier journey to Barcelona due to a 'necessitat que havets havda en vostra persona,' a necessity that you have had in your person; Joan refers to Alfonso's injury or illness as a 'necessity' rather than an 'accident'

## What is this document doing?
- This document reinforces the familial and administrative relationship between members of a royal family.
- The document invokes a symbolic system of honor through the admittance of Alfonso the Younger to a chivalric order.

## Questions
- Why did Joan want the Marquis de Villena in Barcelon? Did he have a particular task in mind?
- Was Alfonso already en route to Barcelona at this time?
- Why has Pere March been sent to deliver both this message and the Cross of Saint George?
- What bodily conditions or circumstances merit the use of the phrase 'necessity of your person' rather than 'misfortune'?

## AI Usage
I started with a transcription from [Gemini](/historians-notebook-s1-episodes/pdfs/e68-gemini-transcription.pdf) and then had [Claude](/historians-notebook-s1-episodes/pdfs/e68-claude-full-research.pdf) do a reconciliation with its own transcription. Claude then created a [translation with footnotes](/historians-notebook-s1-episodes/pdfs/e68-claude-completed-doc.pdf) that address ambiguities and possibilities in the text.

## Additional Notes
The addressee of this letter, Alfonso the Elder, raised his grandson [Enrique de Villena](https://en.wikipedia.org/wiki/Enrique_de_Villena). Enrique became an author of literature and esoterica. He also wrote the earliest surviving report of the *Jocs Florals* poetry competition that Joan imported from Toulouse to Barcelona in 1391.

## Bibliography
- Domenge, Joan. “Les divises de Joan I d’Aragó (1350-96).” *Miscel·lània d’art Medieval i Modern* (Rome), 2023, 251–81.
- Villena, Enrique de. *Arte de trovar.* Edited by F. J. Sánchez Cantón and Alvar Gómez de Castro. Visor Libros, 1993.







<br style="clear: both">
<br style="clear: both">
<br style="clear: both">

[← return to episode list](/historians-notebook/)

<br style="clear: both">
<br style="clear: both">
<br style="clear: both">
